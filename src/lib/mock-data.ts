...
export function generateMockJobApplication(appliedDaysAgo?: number): MockJobApplication {
  ...
  const salaryMin = Math.floor(adjustedSalary * 0.9)
  const salaryMax = Math.floor(adjustedSalary * 1.3)
  ...
  const currency = location.includes('UK') || location.includes('London') || location.includes('Manchester') || location.includes('Edinburgh') ? 'GBP' :
                  ...
                  location.includes('Berlin') || location.includes('Munich') || location.includes('Paris') || 
                  location.includes('Amsterdam') || location.includes('Stockholm') || location.includes('Copenhagen') ||
                  location.includes('Zurich') || location.includes('Geneva') || location.includes('Milan') ||
                  location.includes('Warsaw') || location.includes('Prague') || location.includes('Vienna') ? 'EUR' :
                  location.includes('CA') || location.includes('Toronto') || location.includes('Vancouver') || location.includes('Montreal') ? 'CAD' :
                  location.includes('JP') || location.includes('Tokyo') || location.includes('Osaka') ? 'JPY' :
                  location.includes('IN') || location.includes('Bangalore') || location.includes('Mumbai') || location.includes('Delhi') ? 'INR' :
                  location.includes('AU') || location.includes('Sydney') || location.includes('Melbourne') ? 'AUD' :
                  location.includes('SG') || location.includes('Singapore') ? 'SGD' :
                  location.includes('HK') || location.includes('Hong Kong') ? 'HKD' :
                  'USD'
  ...
  return {
    ...
    company,
    location,
    jobPostUrl: Math.random() > 0.25 ? `https://${company.toLowerCase().replace(/[^a-z0-9]/g, '')}.com/careers/${Math.floor(Math.random() * 10000)}` : undefined,
    salaryMin: Math.random() > 0.15 ? (currency === 'JPY' ? salaryMin * 130 : currency === 'INR' ? salaryMin * 75 : salaryMin) : undefined,
    salaryMax: Math.random() > 0.15 ? (currency === 'JPY' ? salaryMax * 130 : currency === 'INR' ? salaryMax * 75 : salaryMax) : undefined,
    salaryCurrency: currency,
    contactName: Math.random() > 0.5 ? `${getRandomElement(firstNames)} ${getRandomElement(lastNames)}` : undefined,
    contactEmail: Math.random() > 0.6 ? `hiring@${company.toLowerCase().replace(/[^a-z0-9]/g, '')}.com` : undefined,
    contactPhone: Math.random() > 0.75 ? generatePhoneNumber(currency) : undefined,
    notes: Math.random() > 0.3 ? generateJobNotes(status, company, jobTitle) : undefined,
    status,
    priority,
    appliedDate,
    tags: [...allSkills, ...jobLevelTags].slice(0, 8) // Limit to 8 tags
  }
}
